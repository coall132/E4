...
def get_user_predictions(
    ...
    predictions = CRUD.get_predictions_for_user(db, user_id)
    
    # Enrichir les prédictions avec les détails des établissements
    results = []
    for pred in predictions:
        pred_dict = schema.Prediction.model_validate(pred).model_dump()
        ids = [item['etab_id'] for item in pred_dict.get("items", [])]
        details_map = CRUD.get_etablissements_details_bulk(db, ids)
        
        items_rich = []
        for it in pred_dict.get("items", []):
            d = details_map.get(int(it["etab_id"]))
            items_rich.append({**it, "details": d})
        pred_dict["items_rich"] = items_rich
