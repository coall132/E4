...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    box.innerHTML = `
      <div class="mb-2 text-muted">latency: ${data.latency_ms ?? 'NA'} ms — model: ${data.model_version ?? 'dev'}</div>
        ...
        ${items.map(it => `
          <div class="list-group-item">
            <div class="d-flex justify-content-between">
              ...
              <div class="text-end">
                <div class="badge bg-primary">score ${(it.score ?? 0).toFixed(4)}</div>
                ${it.details?.rating ? `<div class="small text-muted">rating ${(it.details.rating).toFixed(1)}</div>` : ''}
              </div>
            </div>
          </div>
        `).join('')}
